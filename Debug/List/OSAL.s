...
//    List file    =  
//        C:\Users\TimT\Documents\SandBox\ZIGBEE_WATCH_dev\Debug\List\OSAL.s
//
///////////////////////////////////////////////////////////////////////////////
...
// 1167     HAL_ENTER_CRITICAL_SECTION(intState);
        BL       IntMasterDisable
        ANDS     R0,R0,#0x1
        EORS     R0,R0,#0x1
// 1168     events = tasksEvents[idx];
        LDR.N    R1,??DataTable9_3
        LDR      R1,[R1, #+0]
        UXTB     R4,R4            ;; ZeroExt  R4,R4,#+24,#+24
        LDRH     R5,[R1, R4, LSL #+1]
// 1169     tasksEvents[idx] = 0;  // Clear the Events for this task.
        MOVS     R1,#+0
        LDR.N    R2,??DataTable9_3
        LDR      R2,[R2, #+0]
        UXTB     R4,R4            ;; ZeroExt  R4,R4,#+24,#+24
        STRH     R1,[R2, R4, LSL #+1]
// 1170     HAL_EXIT_CRITICAL_SECTION(intState);
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        CMP      R0,#+0
        BEQ.N    ??osal_run_system_4
        BL       IntMasterEnable
// 1171 
// 1172     activeTaskID = idx;
??osal_run_system_4:
        LDR.N    R0,??DataTable9_4
        STRB     R4,[R0, #+0]
// 1173     events = (tasksArr[idx])( idx, events );  //This is a function call to a callback whose address is in tasksArr
        MOVS     R1,R5
        ...
        BLX      R2
        MOVS     R5,R0
// 1174     activeTaskID = TASK_NO_TASK;
        MOVS     R0,#+255
        LDR.N    R1,??DataTable9_4
        STRB     R0,[R1, #+0]
// 1175 
// 1176     HAL_ENTER_CRITICAL_SECTION(intState);
        BL       IntMasterDisable
        ANDS     R0,R0,#0x1
        EORS     R0,R0,#0x1
// 1177     tasksEvents[idx] |= events;  // Add back unprocessed events to the current task.
        LDR.N    R1,??DataTable9_3
        LDR      R1,[R1, #+0]
        UXTB     R4,R4            ;; ZeroExt  R4,R4,#+24,#+24
        LDRH     R1,[R1, R4, LSL #+1]
        ...
        UXTB     R4,R4            ;; ZeroExt  R4,R4,#+24,#+24
        STRH     R1,[R2, R4, LSL #+1]
// 1178     HAL_EXIT_CRITICAL_SECTION(intState);
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        CMP      R0,#+0
        BEQ.N    ??osal_run_system_5
        BL       IntMasterEnable
// 1179   }
// 1180 #if defined( POWER_SAVING )
// 1181   else  // Complete pass through all task events with no activity?
// 1182   {
// 1183     osal_pwrmgr_powerconserve();  // Put the processor/system into sleep
// 1184   }
// 1185 #endif
// 1186 
// 1187   /* Yield in case cooperative scheduling is being used. */
// 1188 #if defined (configUSE_PREEMPTION) && (configUSE_PREEMPTION == 0)
// 1189   {
// 1190     osal_task_yield();
// 1191   }
// 1192 #endif
// 1193 }
??osal_run_system_5:
...
// 1194 
// 1195 /*********************************************************************
// 1196  * @fn      osal_buffer_uint32
// 1197  *
// 1198  * @brief
// 1199  *
// 1200  *   Buffer an uint32 value - LSB first.
// 1201  *
// 1202  * @param   buf - buffer
// 1203  * @param   val - uint32 value
// 1204  *
// 1205  * @return  pointer to end of destination buffer
// 1206  */

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
// 1207 uint8* osal_buffer_uint32( uint8 *buf, uint32 val )
// 1208 {
// 1209   *buf++ = BREAK_UINT32( val, 0 );
osal_buffer_uint32:
        STRB     R1,[R0, #+0]
        ADDS     R0,R0,#+1
// 1210   *buf++ = BREAK_UINT32( val, 1 );
        LSRS     R2,R1,#+8
        STRB     R2,[R0, #+0]
        ADDS     R0,R0,#+1
// 1211   *buf++ = BREAK_UINT32( val, 2 );
        LSRS     R2,R1,#+16
        STRB     R2,[R0, #+0]
        ADDS     R0,R0,#+1
// 1212   *buf++ = BREAK_UINT32( val, 3 );
        LSRS     R1,R1,#+24
        STRB     R1,[R0, #+0]
        ADDS     R0,R0,#+1
// 1213 
...
// 1216 
// 1217 /*********************************************************************
// 1218  * @fn      osal_buffer_uint24
// 1219  *
// 1220  * @brief
// 1221  *
// 1222  *   Buffer an uint24 value - LSB first. Note that type uint24 is
// 1223  *   typedef to uint32 in comdef.h
// 1224  *
// 1225  * @param   buf - buffer
// 1226  * @param   val - uint24 value
// 1227  *
// 1228  * @return  pointer to end of destination buffer
// 1229  */

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
// 1230 uint8* osal_buffer_uint24( uint8 *buf, uint24 val )
// 1231 {
// 1232   *buf++ = BREAK_UINT32( val, 0 );
osal_buffer_uint24:
        STRB     R1,[R0, #+0]
        ADDS     R0,R0,#+1
// 1233   *buf++ = BREAK_UINT32( val, 1 );
        LSRS     R2,R1,#+8
        STRB     R2,[R0, #+0]
        ADDS     R0,R0,#+1
// 1234   *buf++ = BREAK_UINT32( val, 2 );
        LSRS     R1,R1,#+16
        STRB     R1,[R0, #+0]
        ADDS     R0,R0,#+1
// 1235 
...
// 1238 
// 1239 /*********************************************************************
// 1240  * @fn      osal_isbufset
// 1241  *
// 1242  * @brief
// 1243  *
// 1244  *   Is all of the array elements set to a value?
// 1245  *
// 1246  * @param   buf - buffer to check
// 1247  * @param   val - value to check each array element for
// 1248  * @param   len - length to check
// 1249  *
// 1250  * @return  TRUE if all "val"
// 1251  *          FALSE otherwise
// 1252  */

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
// 1253 uint8 osal_isbufset( uint8 *buf, uint8 val, uint8 len )
// 1254 {
osal_isbufset:
        PUSH     {R4}
// 1255   uint8 x;
...
// 1263   {
// 1264     // Check for non-initialized value
// 1265     if ( buf[x] != val )
        UXTB     R3,R3            ;; ZeroExt  R3,R3,#+24,#+24
        LDRB     R4,[R3, R0]
        UXTB     R1,R1            ;; ZeroExt  R1,R1,#+24,#+24
...
// 1276  * @brief
// 1277  *
// 1278  *   This function returns the task ID of the current (active) task.
// 1279  *
// 1280  * @param   void
// 1281  *
// 1282  * @return   active task ID or TASK_NO_TASK if no task is active
// 1283  */

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
// 1284 uint8 osal_self( void )
// 1285 {
// 1286   return ( activeTaskID );
osal_self:
        LDR.N    R0,??DataTable9_4
        LDRB     R0,[R0, #+0]
        BX       LR               ;; return
